...
- `generate_required_documents` 는 심사자가 정상 응답을 바로 확인할 수 있게 성공 가능한 단계 조합으로만 예시를 작성했다.
